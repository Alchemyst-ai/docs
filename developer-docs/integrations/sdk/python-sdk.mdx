...
results = client.v1.context.search(
    ...
    body_metadata={          # Optional - can be None
...
| **403** | `Forbidden` | Accessing a `scope` (e.g., `user_123`) you don't have permission for. | Verify the `user_id` matches the authenticated session. |
