...
9. Save — Cursor will trigger OAuth discovery and open `https://mcp.getalchemystai.com/auth/login`. Paste your Alchemyst API key there once to verify (create one at **[Alchemyst Platform](https://platform.getalchemystai.com)** if needed). Fallback without OAuth: add `"headers": { "Authorization": "Bearer YOUR_ALCHEMYST_PLATFORM_API_KEY" }`.
...
| "Authorization failed" | OAuth not completed or API key invalid | Complete the OAuth browser login and paste a valid API key, or regenerate from [**Alchemyst Dashboard**](https://getalchemystai.com?utm_source=docs.getalchemystai.com&utm_campaign=docs&utm_medium=web) |
