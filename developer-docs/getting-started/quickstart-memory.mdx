...
result = alchemyst.v1.context.search(
    limit=10,
    body_metadata={"group_name": ["preferences", "alice"]}
)
...
def chat_with_memory(prompt: str, user_id: str, session_id: str):
    ...
    memory = alchemyst.v1.context.search(
        limit=10,
        body_metadata={"group_name": [session_id, user_id]}
    )
...
```typescript
const memories = await client.v1.context.search({
  limit: 10,
  body_metadata: {"group_name": ["chat_456", "user_123"]}
});
...
await client.v1.context.add({
  documents: [{ content: "Test memory: The user likes pizza" }],
  contextType: "conversation",
  metadata: {
    groupName: ["test_session", "test_user"]
  }
});
...
const result = await client.v1.context.search({
  limit: 10,
  body_metadata: {"group_name": ["test_session", "test_user"]}
});
